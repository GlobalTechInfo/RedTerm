...
case "$ARCH" in
  x86_64)
    ...
    docker import /tmp/artix-bs.tar artix-img:latest
    CID=$(docker create artix-img:latest /bin/true)
    docker export "$CID" | sudo tar -xf - -C "$ROOTFS"
    docker rm "$CID"
    docker rmi artix-img:latest 2>/dev/null || true
  ...
  aarch64)
    ...
    docker import /tmp/artix-arm.tar.gz artix-img:latest
    CID=$(docker create artix-img:latest /bin/true)
    docker export "$CID" | sudo tar -xf - -C "$ROOTFS"
    docker rm "$CID"
    docker rmi artix-img:latest 2>/dev/null || true
